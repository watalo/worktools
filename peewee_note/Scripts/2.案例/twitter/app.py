...
class User(BaseModel):
    ...
    def following(self):
        ...
        return (User
                .select()
                .join(Relationship, on=Relationship.to_user) # on用来判断是否join，Relation.to_user = True
                .where(Relationship.from_user == self)       # 其他用法：on = (User.id == Message.user_id)
...
class Relationship(BaseModel):
    ...
    class Meta: # Model.Meta有很多不同的属性，indexes是其中之一，用来建立索引
        indexes = (
